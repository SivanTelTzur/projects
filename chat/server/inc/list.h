 ...
 * 
 *  @author Author SivanTelTzur (tz.sivan@gmail.com) 
 * 
 *  @bug No known bugs.
 */ 
 ...
 *
 * @params _list : A previously created List ADT returned via ListCreate
 * @params _pItem: To store a pointer to the removed item in,  if pointer is NULL pop anyway.
 * @retval LIST_SUCCESS on success
 * @retval LIST_UNINITIALIZED_ERROR if the list is not initialized
 * @returns LIST_UNDERFLOW if the list is empty.
 *
 */
 ...
 * @returns 0 if List is empty or  if the list is not initialized.
 */
