...
namespace advcpp
{
TcpServer::TcpServer(size_t a_port, MsgFuncP a_getMsgFunction, newClientFuncP a_newClientFunc) 
: m_listenSock(createSocket())
, m_port(a_port)
, m_clients()
, m_getMsgFunction(a_getMsgFunction)
, m_newClientFunc(a_newClientFunc)
, m_ipMap()
{
    noBlockinSock();
    setReuseSockOpt();
...
void TcpServer::bindSocket() 
{
    ...
    sin.sin_addr.s_addr = inet_addr("127.8.8.1");//INADDR_ANY;
...
void TcpServer::createNewClient(fd_set& a_master) 
{
    ...
    std::string clientIp(inet_ntoa(client_sin.sin_addr));
    m_ipMap[clientSock] = clientIp;
    m_newClientFunc->NewClient(clientSock, clientIp);

}


int TcpServer::recvValue(int a_clientSocket) 
{
    int toRead = 4;
    char valueLen[sizeof(int)];
    char * buffer = valueLen; 

    while (toRead > 0){
        ssize_t recv_bytes = recv(a_clientSocket, buffer, toRead, 0);
        if (recv_bytes <= 0 ){
        ...
        //return  *((unsigned int*)valueLen); 
        return ntohl(*(reinterpret_cast<unsigned int*>(valueLen)));
}

void TcpServer::recvMsg(int a_clientSocket, std::string& a_buffer)
{
    a_buffer.clear();
    int bytesToRecv = 0;
    try{
        bytesToRecv = recvValue(a_clientSocket);
    }
    ...
    while (bytesToRecv > 0){
        ssize_t recv_bytes = recv(a_clientSocket, buffer, bytesToRecv, 0);
        if (recv_bytes <= 0){
            delete buffer;
            throw   std::runtime_error("recv failed");  
        }
...
void TcpServer::clientReceive(fd_set& a_master, fd_set& a_temp, int a_activity) 
{
    ...
    while (it != m_clients.end() && a_activity > 0){
        ...
        if (FD_ISSET(currClientSock, &a_temp)){
            std::string buffToFill;
            try{
                recvMsg(currClientSock, buffToFill); 
            }
            catch (const std::runtime_error& a_ex){
                close(currClientSock);
                FD_CLR(currClientSock, &a_master);
                m_ipMap.erase(currClientSock);
                it = m_clients.erase(it);
                StartLogger().Message("client closed\n");
                --a_activity;
                continue;
            }
            m_getMsgFunction->ProcessMsg(buffToFill, currClientSock, m_ipMap[currClientSock]);
            ++it;         
            --a_activity;
        }else {
            ++it;
...
//                FD_CLR(currClientSock, &a_master);
//                it = m_clients.erase(it);
//                StartLogger().Message("message was not received client closed\n");
//            }else{
