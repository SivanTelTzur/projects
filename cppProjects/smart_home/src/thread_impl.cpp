...
ThreadImplemant::ThreadImplemant(SharedPtr<ObjThreadFunc> a_spObjThreadFunc) 
: m_isJoined(false)
, m_isDetached(false) 
{
    CloningProxy<ObjThreadFunc> proxy(*a_spObjThreadFunc);
    proxy.Wait();
    const int TRIES = 3;
    unsigned int sleepTimeUs = 80;
    for (int i = 0; i < TRIES; ++i){
        int status = pthread_create(&m_thread, 0, ActivateThreadFuncSharedPtr, &proxy);
...
void* ThreadImplemant::ActivateThreadFuncSharedPtr(void* a_objFunc) 
{
    assert(a_objFunc);
    CloningProxy<ObjThreadFunc>* objProxy =  static_cast<CloningProxy<ObjThreadFunc>*>(a_objFunc);

    SharedPtr<ObjThreadFunc> spObjFunc(objProxy->Clone());
    objProxy->Signal();
    try{
        spObjFunc->operator()();
    }
