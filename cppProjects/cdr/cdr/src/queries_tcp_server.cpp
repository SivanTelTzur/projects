...
namespace advcpp
{
QueriesTcpServer::QueriesTcpServer(size_t a_port, MtWaitingQueue<Query>& a_msgQueue) 
...
QueriesTcpServer::~QueriesTcpServer() 
{
...
void QueriesTcpServer::Run() 
{
    runServer();  
}

void QueriesTcpServer::Receive(char* a_buffer) 
{
    
}

void QueriesTcpServer::Send(const char* a_buffer, size_t a_size, int a_client) 
{
...
int QueriesTcpServer::createSocket() 
{
    const int TRIES = 3;
    ...
    throw QueriesServerLimitsException();    
}

void QueriesTcpServer::noBlockinSock() 
{
...
void QueriesTcpServer::setReuseSockOpt() 
{
    int optval = 1;
...
void QueriesTcpServer::bindSocket() 
{
    ...
    if (0 > bind(m_listenSock, reinterpret_cast<struct sockaddr *>(&sin), sizeof(sin)))
    {
        std::cerr << errno;
        StartLogger().Message("server - bindSocket d- bind failed\n");
        std::terminate();
    }    
}

void QueriesTcpServer::listenSock() 
{
...
void QueriesTcpServer::runServer() 
{
    fd_set master;
    FD_ZERO(&master);
...
void QueriesTcpServer::createNewClient(fd_set& a_master) 
{
    int clientSock;
...
void QueriesTcpServer::clientReceive(fd_set& a_master, fd_set& a_temp, int a_activity) 
{
    ...
    while (it != m_clients.end() && a_activity > 0){
        ...
        if (FD_ISSET(currClientSock, &a_temp)){
            ...
            if (0 >= read_byte){
                ...
                Query newQuery = {currClientSock, buffer};
                m_msg.Enqueue(newQuery);
                ++it;
            }            
            --a_activity;
        }else{
...
const char* QueriesServerLimitsException::what() const throw()
{
    return "Query server system limits has reached - socket failed";
} 
